...
class Goods {
    ...
    async getGoods(req, res, next) {
        try {
            ...
            const goods = await Food.find().sort({id: -1}).skip(Number(pageSize) * (Number(current) - 1)).limit(Number(pageSize)).exec();
    ...
    async addFood(req, res, next){
        ...
        form.parse(req, async (err, fields, files) => {
            ...
            let restaurant;
            try{
                category = await Category.findOne({id: fields.category_id});
